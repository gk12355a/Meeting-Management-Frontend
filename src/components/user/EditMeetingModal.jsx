  ...
  const executeUpdate = async (values, mode) => {
    ...
    try {
      ...
      if (res.data?.status === "PENDING_APPROVAL") {
        toast.info("📝 Yêu cầu chỉnh sửa đã được gửi và đang chờ Admin phê duyệt.", { autoClose: 5000 });
      } else {
        toast.success(mode === 'SERIES' ? "Cập nhật chuỗi cuộc họp thành công!" : "Cập nhật cuộc họp thành công!");
      }
      ...
      >
        <Card className="shadow-none bg-white dark:bg-[#1e293b] border-none dark:text-gray-100" styles={{ body: { padding: 0 } }}>
          ...
          >
            ...
            {/* ROOM SELECT */}
            <Form.Item name="roomId" label="Phòng họp" rules={[{ required: true }]}>
              <Select
                    ...
                    >
                      {({ getFieldValue }) => 
                        getFieldValue('frequency') === 'WEEKLY' ? (
                          <Form.Item name="daysOfWeek" label="Lặp lại vào các thứ" rules={[{ required: true, message: "Chọn ít nhất một ngày" }]}>
      ...
      >
        <div className="p-2">
          ...
          <div className="mt-4 text-right">
             <Button type="text" onClick={() => setConfirmModalOpen(false)}>Hủy bỏ</Button>
          </div>
        </div>
      </Modal>
    </>
